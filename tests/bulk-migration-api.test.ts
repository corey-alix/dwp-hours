...
describe("Bulk Data Migration API", () => {
  it("should successfully import bulk data for a new employee", async () => {
    ...
    const newEmployee = employees.find(
      (emp: any) => emp.identifier === "test-bulk-import@example.com",
    );
  ...
  it("should handle duplicate data gracefully", async () => {
    const bulkData = {
      ...
      monthlyHours: [{ month: "2025-01", hours: 160 }],
      ptoEntries: [{ date: "2025-01-15", hours: 8, type: "PTO" }],
    };

    // First import
    await request(app).post("/api/migrate/bulk").send(bulkData);
  ...
  it("should validate input data", async () => {
    const response = await request(app).post("/api/migrate/bulk").send({
