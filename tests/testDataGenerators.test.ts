...
import {
  ...
  extractMonthlyPTOHours,
  extractMonthlyWorkDays,
  extractMonthCellRange,
  extractHireDate,
  extractYear,
  extractEmployeeName,
  extractSickHoursStatus,
  extractLegend,
  extractDailyRates,
  extractPreviousYearsCarryOverPTO,
  extractEmployeeSignoffs,
  extractAdminSignoffs,
} from "../shared/testDataGenerators";
import fs from "fs";
...
describe("testDataGenerators", () => {
  ...
  it("should generate the same JSON as import-tests.json", () => {
    const expected = JSON.parse(
      fs.readFileSync(
        path.join(__dirname, "data", "import-tests.json"),
        "utf-8",
      ),
    );
  ...
  it("should extract employee signoffs from X42 to X53", () => {
    ...
    expect(employeeSignoffs).toEqual([
      "CA",
      "CA",
      "CA",
      "CA",
  ...
  it("should extract daily rates from F42 to F53", () => {
    ...
    expect(dailyRates).toEqual([
      0.68, 0.68, 0.71, 0.71, 0.71, 0.71, 0.71, 0.71, 0.71, 0.71, 0.71, 0.71,
    ]);
  });
});
