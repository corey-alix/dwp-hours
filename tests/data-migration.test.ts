...
describe("Data Migration POC - Excel Parsing", () => {
  it("should read and parse PTO calculation data from Excel file", async () => {
    ...
    if (!fs.existsSync(filePath)) {
      console.warn(
        'Excel file not found, skipping test. Please provide "Corey Alix 2025.xlsx"',
      );
      return;
    }
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      if (
        row.some(
          (cell) =>
            cell && cell.toString().toLowerCase().includes("pto calculation"),
        )
      ) {
        ptoSectionStart = i;
    ...
    for (
      ...
      i < jsonData.length && ptoData.length < 12;
      i++
    ) {
      const row = jsonData[i] as any[];
      ...
      if (
        ...
        const data = {
          month,
          workDays:
            typeof workDays === "number" ? workDays : parseInt(workDays) || 0,
          dailyRate:
            typeof dailyRate === "number"
              ? dailyRate
              : parseFloat(dailyRate) || 0,
          accrued:
            typeof accrued === "number" ? accrued : parseFloat(accrued) || 0,
          carryover:
            typeof carryover === "number"
              ? carryover
              : parseFloat(carryover) || 0,
          subtotal:
            typeof subtotal === "number" ? subtotal : parseFloat(subtotal) || 0,
          usedHours:
            typeof usedHours === "number"
              ? usedHours
  ...
  it("should confirm September 1st cell color differs from September 2nd using ExcelJS", async () => {
    ...
    const findCellForDate = (month: string, day: number): ExcelJS.Cell => {
      ...
      console.log(
        `Cell for ${month} ${day}: row ${weekRow}, col ${dayCol}, value:`,
        cell.value,
      );
      return cell;
    };
  ...
  it("should extract PTO type colors from legend and count matching calendar cells", async () => {
    ...
    for (let i = 0; i < ptoTypes.length; i++) {
      ...
      if (cell.fill && cell.fill.type === "pattern" && cell.fill.fgColor) {
        ...
        if (color) {
          ...
          const ptoType =
            ptoTypes[i] === "Full PTO" || ptoTypes[i] === "Partial PTO"
              ? "PTO"
              : ptoTypes[i];
          ...
          if (!ptoTypeColors[ptoType]) {
            ...
            console.log(
              `${ptoTypes[i]} (${ptoType}): ${color} (skipped, already have color for ${ptoType})`,
            );
          }
        }
      }
    ...
    for (let row = 6; row <= 37; row++) {
      for (let col = 2; col <= 24; col++) {
        ...
        if (cell.fill && cell.fill.type === "pattern" && cell.fill.fgColor) {
          ...
          if (cellColor) {
            console.log(
              `Cell ${String.fromCharCode(64 + col)}${row}: color ${cellColor}, value: ${cell.value}`,
            );
            // Find the closest matching PTO type color
            for (const [ptoType, legendColor] of Object.entries(
              ptoTypeColors,
            )) {
    ...
    const totalMatches = Object.values(calendarMatches).reduce(
      (sum, count) => sum + count,
      0,
    );
  ...
  it("should mine and document complete spreadsheet layout information", async () => {
    ...
    if (legendRow !== -1) {
      console.log(
        `   Location: Row ${legendRow}, Column ${legendCol} (Cell ${String.fromCharCode(64 + legendCol)}${legendRow})`,
      );
        ...
        if (!existingMonth) {
          ...
          if (monthColumns.length > 0) {
            ...
            monthHeaders.push({
              month,
              startCol,
              endCol: Math.min(endCol, 24),
            });
            console.log(
              `     ${month}: Columns ${startCol}-${endCol} (${String.fromCharCode(64 + startCol)}-${String.fromCharCode(64 + endCol)})`,
            );
          }
        }
      }
    ...
    worksheet.eachRow((row, rowNumber) => {
      row.eachCell((cell) => {
        if (
          cell.value &&
          typeof cell.value === "string" &&
          cell.value.toLowerCase().includes("pto calculation")
        ) {
    ...
    if (ptoSectionRow !== -1) {
      ...
      console.log(
        "   Data columns: Month, Work Days, Daily Rate, Accrued, Carryover, Subtotal, Used Hours, Remaining",
      );
    }
  ...
  it("should scan and document all month locations in the spreadsheet", async () => {
    ...
    monthLocations.forEach((location) => {
      console.log(
        `  ${location.month}: Row ${location.row}, Column ${location.col} (Cell ${location.cell})`,
      );
    });
  ...
  it("should scan and document acknowledgement sections", async () => {
    ...
    const employeeAcknowledgements: {
      row: number;
      value: any;
      cell: string;
    }[] = [];
    ...
    const nonEmptyAdmin = adminAcknowledgements.filter(
    ...
    const nonEmptyEmployee = employeeAcknowledgements.filter(
      (a) => a.value !== null && a.value !== undefined && a.value !== "",
    );
    console.log(
      `  Employee acknowledgements found: ${nonEmptyEmployee.length}`,
    );
  ...
  it("should analyze and document D42-W53 data section with headers in rows 40-41", async () => {
    ...
    console.log("\n=== STRUCTURE ANALYSIS ===");
    console.log(
      `Data section dimensions: ${dataSection.length} rows x ${dataSection[0]?.length || 0} columns`,
    );
    ...
    for (let col = 0; col < (dataSection[0]?.length || 0); col++) {
      ...
      for (let row = 0; row < dataSection.length; row++) {
        if (
          dataSection[row][col] !== null &&
          dataSection[row][col] !== undefined &&
          dataSection[row][col] !== ""
        ) {
          columnCounts[col]++;
        }
    ...
    Object.entries(columnCounts).forEach(([col, count]) => {
      ...
      console.log(
        `  Column ${colLetter} (${parseInt(col) + 4}): ${count} non-empty cells`,
      );
    });
