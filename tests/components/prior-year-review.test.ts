...
describe('PriorYearReview Component', () => {
    ...
    beforeEach(async () => {
        ...
        const { PriorYearReview } = await import('../../client/components/prior-year-review/index.js');
    ...
    describe('Data Rendering', () => {
        ...
        it('should render calendar grids for each month', () => {
            component.data = mockData;
            const calendarGrids = component.shadowRoot?.querySelectorAll('.calendar-grid');
    ...
    describe('Data Property', () => {
        it('should update rendering when data changes', () => {
            ...
            const data: PTOYearReviewResponse = {
                ...
                months: [{
                    month: 1,
                    ptoEntries: [{ date: '2025-01-01', type: 'PTO' as const, hours: 8 }],
                    summary: {
                        totalDays: 31,
                        ptoHours: 8,
                        sickHours: 0,
            ...
            expect(component.shadowRoot?.querySelectorAll('.month-card').length).toBe(12);
        });
    ...
    describe('CSS Styling', () => {
        ...
        it('should apply correct colors to PTO types', () => {
            ...
            const styleContent = component.shadowRoot?.querySelector('style')?.textContent;
            expect(styleContent).toContain('.type-PTO { background: var(--color-pto-vacation); }');
            expect(styleContent).toContain('.type-Sick { background: var(--color-pto-sick); }');
            expect(styleContent).toContain('.type-Bereavement { background: var(--color-pto-bereavement); }');
            expect(styleContent).toContain('.type-Jury-Duty { background: var(--color-pto-jury-duty); }');
        });
    });
});