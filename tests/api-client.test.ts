...
describe('APIClient', () => {
    ...
    describe('createPTOEntry', () => {
        it('should create a single PTO entry', async () => {
            const mockResponse = {
                ...
                ptoEntry: { id: 1, employeeId: 1, date: '2026-03-06', type: 'PTO', hours: 4 },
                ptoEntries: [{ id: 1, employeeId: 1, date: '2026-03-06', type: 'PTO', hours: 4 }]
            };
            fetchMock.mockResolvedValueOnce({
                json: () => Promise.resolve(mockResponse)
            });

            const request = { employeeId: 1, date: '2026-03-06', hours: 4, type: 'PTO' };
        ...
        it('should create multiple PTO entries', async () => {
            const mockResponse = {
                message: 'PTO entries created successfully',
                ptoEntry: { id: 2, employeeId: 1, date: '2026-03-07', type: 'PTO', hours: 8 },
                ptoEntries: [
    ...
    describe('requestAuthLink', () => {
        it('should request authentication link', async () => {
            const mockResponse = { message: 'If the email exists, a magic link has been sent.' };
            fetchMock.mockResolvedValueOnce({
                json: () => Promise.resolve(mockResponse)
            });
