...
import { jsonToWorkbook, workbookToJson, type ExcelData } from '../shared/conversionUtils.ts';
...
function compareJson(a: any, b: any, path: string = ''): string[] {
    ...
    if (a === null || b === null || typeof a !== 'object') {
        mismatches.push(`Value mismatch at ${path}: ${JSON.stringify(a)} vs ${JSON.stringify(b)}`);
        if (mismatches.length >= 10) return mismatches;
        return mismatches;
    }
...
describe('Round-Trip Validation', () => {
    ...
    describe('Excel → JSON → Excel → JSON', () => {
        it('should preserve all data through Excel to JSON to Excel to JSON round-trip (JSON-to-JSON comparison)', async () => {
            ...
            writeFileSync('tests/data/import-tests.json', JSON.stringify(finalJsonData, null, 2), 'utf-8');
        });
    });
});