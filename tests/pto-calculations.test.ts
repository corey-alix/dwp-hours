...
describe("PTO Calculations", () => {
  ...
  it("should calculate PTO status correctly", () => {
    ...
    // annualAllocation = policy rate × workdays. Hired 2024-01-01 → tier 0 (0.65) × 262 = 170.30
    expect(status.employeeId).toBe(1);
    expect(status.annualAllocation).toBeCloseTo(170.3, 1);
    expect(status.availablePTO).toBeCloseTo(140.3, 0); // 170.30 + 10 - 40
    ...
    expect(status.monthlyAccruals[0].hours).toBeCloseTo(14.95, 2); // 0.65 * 23 = 14.95
    expect(status.sickTime.used).toBe(8);
    expect(status.sickTime.remaining).toBe(16); // 24 - 8
    expect(status.ptoTime.allowed).toBeCloseTo(180.3, 0); // 170.30 + 10
    expect(status.ptoTime.used).toBe(40);
    expect(status.ptoTime.remaining).toBeCloseTo(140.3, 0);
    expect(status.bereavementTime.used).toBe(8);
    expect(status.bereavementTime.remaining).toBe(8); // 16 - 8
    expect(status.juryDutyTime.used).toBe(0);
    expect(status.juryDutyTime.remaining).toBe(24);
  ...
  it("should calculate year-end carryover", () => {
    ...
    // With policy-based rates: allocation 170.30 + carryover 10 - used 40 = 140.30
    // But capped at CARRYOVER_LIMIT (80)
    expect(carryover).toBe(80);
  });
  ...
  it("should calculate prorated allocation for new hires", () => {
    ...
    // Policy-based prorated allocation: 0.65 × workdays(2024-06-01, 2024-12-31)
    // June 1, 2024 is Saturday, so counting from Jun 3 Mon through Dec 31
    expect(status.annualAllocation).toBeGreaterThan(90);
    expect(status.annualAllocation).toBeLessThan(120);
    expect(status.availablePTO).toBe(status.annualAllocation); // No usage
