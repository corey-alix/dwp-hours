  ...
  try {
    ...
    if (existingRows.length > 0) {
      ...
      for (const row of existingRows) {
        const [id, employeeId, startDate, endDate, type, hours, createdAt] =
          row as [number, number, string, string, string, number, string];
        ...
        if (workDays.length === 0) {
          log(
            `Warning: No work days found for PTO entry ${id} (${startDate} to ${endDate})`,
          );
          continue;
        }
        ...
        log(
          `Migrated PTO entry ${id}: ${workDays.length} work days, ${hoursPerDay.toFixed(2)} hours each`,
        );
      }
