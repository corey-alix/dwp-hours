...
import { jsonToWorkbook, validateExcelData, type ExcelData } from '../shared/conversionUtils.ts';

/**
 * Main conversion function
 */
async function convertJsonToExcel(inputPath: string, outputPath: string): Promise<void> {
    try {
...
async function main(): Promise<void> {
    ...
    if (args.length < 2) {
        console.error('Usage: node scripts/convert-json-to-excel.mts <input.json> <output.xlsx>');
        process.exit(1);
    }
