 ...
 */
async function convertExcelToJson(inputPath: string, outputPath: string): Promise<void> {
    try {
...
async function main(): Promise<void> {
    ...
    if (args.length < 2) {
        console.error('Usage: tsx scripts/convert-excel-to-json.mts <input.xlsx> <output.json>');
        process.exit(1);
    }
