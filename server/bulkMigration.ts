...
import { VALIDATION_MESSAGES, PTOType, validateDateString, validatePTOType, validateHours } from '../shared/businessRules.js';
...
async function parseExcelFile(filePath: string): Promise<{ data: BulkMigrationData; warnings: string[] }> {
    ...
    for (let row = 42; row <= 53; row++) {
        ...
        } else if (typeof workDaysValue === 'object' && workDaysValue && 'result' in workDaysValue) {
    ...
    for (const monthInfo of monthPositions) {
        const monthIndex = new Date(`${monthInfo.name} 1, ${currentYear}`).getMonth();
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (cell.value == day) { // Only check cells with the correct day number
                // Check for PTO color
                ...
                if (fill && fill.fgColor) {
                    ...
                    if (color) {
                        ...
                        if (ptoType) {
                            const dateStr = dateToString(new Date(currentYear, monthIndex, day));

                            // Validate date
                            const dateError = validateDateString(dateStr);
                            if (dateError) {
                                warnings.push(`Invalid date ${dateStr}: ${VALIDATION_MESSAGES[dateError.messageKey as keyof typeof VALIDATION_MESSAGES]}`);
                                continue;
                            }
                            ...
                            if (typeError) {
                                warnings.push(`Invalid PTO type ${ptoType} for ${dateStr}: ${VALIDATION_MESSAGES[typeError.messageKey as keyof typeof VALIDATION_MESSAGES]}`);
                                continue;
                            }
                            ...
                            if (hoursError) {
                                warnings.push(`Invalid hours for ${dateStr}: ${VALIDATION_MESSAGES[hoursError.messageKey as keyof typeof VALIDATION_MESSAGES]}`);
                                continue;
                            }
...
export async function performFileMigration(
    ...
    const { data: parsedData, warnings: parseWarnings } = await parseExcelFile(data.filePath);
    // Override employee email if provided
    if (data.employeeEmail) {
        parsedData.employeeEmail = data.employeeEmail;
    }
    const result = await performBulkMigration(dataSource, ptoEntryDAL, log, today, isValidDateString, parsedData);
