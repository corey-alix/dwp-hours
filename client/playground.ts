...
export class TestWorkflow {
    ...
    if (ptoCalendar) {
      ...
      ptoCalendar.addEventListener("pto-request-submit", async (event: any) => {
        ...
        try {
          ...
          alert(
            `PTO request submitted successfully! ${requests.length} date(s) requested.`,
          );
          ptoCalendar.clearSelection();
        } catch (error) {
  ...
  private async runWorkflowTest(): Promise<void> {
    try {
      ...
      const ptoTypeSelect = document.getElementById(
        "pto-type",
      ...
      const monthlyHoursSection = document.getElementById(
      ...
      const hoursWorkedInput = document.getElementById(
        "hours-worked",
      ) as HTMLInputElement;
      const hoursForm = document.getElementById(
        "monthly-hours-form",
      ) as HTMLFormElement;
      ...
      const acknowledgementSection = document.getElementById(
      ...
      const ackMonthInput = document.getElementById(
        "ack-month",
      ) as HTMLInputElement;
      const ackForm = document.getElementById(
      ...
      const ptoCalendarSection = document.getElementById(
      ...
      if (testOutput) {
        testOutput.textContent =
          "❌ Workflow test failed: " + (error as Error).message;
      }
    }
  }
}
