...
export class PtoJuryDutyCard extends BaseComponent {
  ...
  protected render(): string {
    if (!this.data) {
      return `<style>${CARD_CSS}</style>${renderCardShell("Jury Duty", "<div>Loading...</div>")}`;
    }
    const body = renderBucketBody({
      ...
      showNegativeFormatting: false,
    });
    return `<style>${CARD_CSS}</style>${renderCardShell("Jury Duty", body)}`;
  }
