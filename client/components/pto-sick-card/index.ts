...
export class PtoSickCard extends BaseComponent {
  ...
  protected render(): string {
    if (!this.data) {
      return `<style>${CARD_CSS}</style>${renderCardShell("Sick Time", "<div>Loading...</div>")}`;
    }
    ...
    return `<style>${CARD_CSS}</style>${renderCardShell("Sick Time", body)}`;
  }
