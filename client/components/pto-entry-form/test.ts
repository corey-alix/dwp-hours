...
export function playground() {
    ...
    addEventListener(ptoForm, 'pto-submit', (e: CustomEvent) => {
        ...
        const detail = e.detail as { ptoRequest?: { ptoType: string; hours: number }; requests?: { type: string; hours: number; date: string }[] };
        if (detail.ptoRequest) {
            appendLog(`Form submit: ${detail.ptoRequest.ptoType} - ${detail.ptoRequest.hours} hours`);
            return;
        }
        if (detail.requests) {
