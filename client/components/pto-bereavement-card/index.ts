...
export class PtoBereavementCard extends BaseComponent {
  ...
  protected render(): string {
    if (!this.data) {
      return `<style>${CARD_CSS}</style>${renderCardShell("Bereavement", "<div>Loading...</div>")}`;
    }
    const body = renderBucketBody({
      data: this.data,
      entries: this.entries,
      expanded: this.expanded,
      entryType: "Bereavement",
      fullEntries: this.fullEntries,
      showNegativeFormatting: true,
    });
    return `<style>${CARD_CSS}</style>${renderCardShell("Bereavement", body)}`;
  }
