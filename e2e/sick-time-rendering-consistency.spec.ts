...
test.describe('Sick Time Rendering Consistency', () => {
    test('should not show sick time for 2/2/2026 in dashboard if no data exists, but report should match data', async ({ page }) => {
        ...
        const hasFeb2InDashboard = dashboardSickEntries.some(entry =>
            entry.includes('2026-02-02') || entry.includes('2/2/2026')
        );
        ...
        const hasFeb17InDashboard = dashboardSickEntries.some(entry =>
            entry.includes('2026-02-17') || entry.includes('2/17/2026')
        );
