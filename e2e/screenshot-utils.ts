...
export async function compareAndUpdateScreenshot(
  ...
  try {
    ...
    if (
      newPng.width !== baselinePng.width ||
      newPng.height !== baselinePng.height
    ) {
      console.warn(
        `Dimensions differ (${newPng.width}x${newPng.height} vs ${baselinePng.width}x${baselinePng.height}), updating baseline: ${baselinePath}`,
      );
      fs.writeFileSync(baselinePath, newBuffer);
      return;
    }

    const diffCount = computePixelDiff(
      newPng.data,
      baselinePng.data,
      colorThreshold,
    );
