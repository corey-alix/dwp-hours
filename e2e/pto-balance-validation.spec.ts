...
test.describe('PTO Balance Validation', () => {
    ...
    test('should prevent PTO submission when balance is insufficient', async ({ page }) => {
        ...
        await expect(hoursError).toHaveText('PTO request exceeds available PTO balance');
        ...
        page.on('response', response => {
            if (response.url().includes('/api/pto') && response.request().method() === 'POST') {
    ...
    test('should validate balance on form changes', async ({ page }) => {
        ...
        await expect(form.locator('#hours-error')).toHaveText('PTO request exceeds available PTO balance');
    ...
    test('should handle exact balance match', async ({ page }) => {
        ...
        expect(errorText === '' || errorText === 'PTO request exceeds available PTO balance').toBe(true);
    });
});