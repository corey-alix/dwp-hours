...
test.describe('File-Based Data Migration API', () => {
    ...
    test('should execute full migration from Excel file and verify data integrity', async ({ page }) => {
        ...
        const employeeResponse = await page.request.get(`/api/employees/${employeeId}`);
        ...
        const monthlyHoursResponse = await page.request.get(`/api/employees/${employeeId}/monthly-hours`);
        ...
        const ptoResponse = await page.request.get(`/api/employees/${employeeId}/pto-entries`);
        ...
        if (ptoEntries.length > 0) {
            ptoEntries.forEach((entry: any) => {
                ...
                expect(['Sick', 'PTO', 'Bereavement', 'Jury Duty']).toContain(entry.type);
