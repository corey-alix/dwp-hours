...
test("pto-entry-form component test", async ({ page }) => {
  ...
  await page.waitForFunction(
    () => customElements.get("pto-entry-form") !== undefined,
  );
  ...
  await expect(
    page.locator("pto-entry-form").locator("#start-date"),
  ).toBeVisible();
  await expect(
    page.locator("pto-entry-form").locator("#end-date"),
  ).toBeVisible();
  await expect(
    page.locator("pto-entry-form").locator("#pto-type"),
  ).toBeVisible();
  await expect(page.locator("pto-entry-form").locator("#hours")).toBeVisible();
  await expect(
    page.locator("pto-entry-form").locator("#submit-btn"),
  ).toBeVisible();
  await expect(
    page.locator("pto-entry-form").locator("#cancel-btn"),
  ).toBeVisible();
  ...
  await expect(
    page.locator("pto-entry-form").locator("#start-date-error"),
  ).toHaveText("This field is required");
  await expect(
    page.locator("pto-entry-form").locator("#end-date-error"),
  ).toHaveText("This field is required");
  await expect(
    page.locator("pto-entry-form").locator("#pto-type-error"),
  ).toHaveText("This field is required");
  await expect(
    page.locator("pto-entry-form").locator("#hours-error"),
  ).toHaveText("This field is required");
  ...
  await page
    .locator("pto-entry-form")
    .locator("#start-date")
    .fill(startDateStr);
  await page
    .locator("pto-entry-form")
    .locator("#pto-type")
    .selectOption("Full PTO");
...
test("pto-entry-form weekend warning test", async ({ page }) => {
  ...
  await expect(
    page.locator("pto-entry-form").locator("#start-date-error"),
  ).toHaveText("Date must be a weekday (Monday to Friday)");
...
test("pto-entry-form spillover calculation test", async ({ page }) => {
  ...
  await expect(page.locator("pto-entry-form").locator("#end-date")).toHaveValue(
    expectedEndDateStr,
  );
...
test("pto-entry-form dynamic field behavior test", async ({ page }) => {
  ...
  const hoursLabel = page
    .locator("pto-entry-form")
    .locator('label[for="hours"]');
...
test("pto-entry-form calendar interaction test", async ({ page }) => {
  ...
  await page.waitForFunction(() => {
    ...
    return (
      output &&
      output.textContent &&
      output.textContent.includes("Calendar submit")
    );
  });
