...
test('confirmation-dialog component test', async ({ page }) => {
    ...
    expect(logs.some(log => log.text.includes('Starting Confirmation Dialog playground test'))).toBe(true);
    ...
    await expect(shadowMessage).toContainText('Are you sure you want to delete this item?');
